...
class Dataset(torch.utils.data.IterableDataset):
    def __init__(self, path, shuffle_pairs=True, augment=False):
        ...
        if self.augment:
            ...
            self.transform = transforms.Compose([
                ...
                # Cambio para buscar optimización
    ...
    def create_pairs(self):
        ...
        if self.shuffle_pairs:
            ...
            # np.random.seed(1) --> No queremos que sea determinista para una evaluación k-test
            np.random.seed(int(time.time()))
    ...
    def __iter__(self):
        ...
        for idx, idx2 in zip(self.indices1, self.indices2):
            ...
            #image1 = torch.cuda.FloatTensor(Image.open(image_path1).convert("RGB"), device='cuda')
            #image2 = torch.cuda.FloatTensor(Image.open(image_path2).convert("RGB"), device='cuda')
            ...
            # Cambio para probar rendimiento
